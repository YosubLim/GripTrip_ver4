...
$(function() {
	...
	//검색할때 선택되어진 카테고리
	if
		(${select==null} || ${select==""}) $('#category').val("all").attr("selected","selected");
	else 
...
<body>
      ...
      <div class="row">
         ...
         <div class="col-md-1">
            <select id="category" onchange="category_selected()" class="form-control" style="position: relative;top: 33px;height: 40px;">
              <option value="all" class="cate">전체</option>
              <option value="title" class="cate">제목</option>
              <option value="content" class="cate">내용</option>
              <option value="location" class="cate">장소</option>
            </select>
         ...
         <div class="col-md-6">
            <table style="width: 100%;text-align: center;">
               <tr style="height: 10px;">
                  <!-- <td style="background-color: #b71010;"><a onclick="selectButton('all')" style="color: white; font-size: 30px;">All</a></td>
                  <td><a onclick="selectButton('book')" style="font-size: 25px;">Storybook</a></td>
                  <td><a onclick="selectButton('story')" style="font-size: 25px;">Story</a></td> -->
                  <td style="border-bottom: 2px solid black;"><a onclick="selectButton('all')" style="color: black;font-size: 22px;">All</a></td>
                  <td><a onclick="selectButton('book')" style="color: black;font-size: 22px;">Storybook</a></td>
                  <td><a onclick="selectButton('story')" style="color: black;font-size: 22px;">Story</a></td>
               </tr>
            </table>
         </div>
         <div class="col-md-3"></div>
   ...
   <br/>
<c:choose>
	<c:when test="${fn:length(bookList)==0 && fn:length(storyList)==0}">
		<h2 align="center">공개된 스토리북과 스토리가 없습니다.</h2>
	</c:when>
	<c:otherwise>
		<c:choose>
      ...
      <c:otherwise>
         <h2 align="center">공개된 스토리북이 없습니다.</h2>
      </c:otherwise>
   </c:choose>
   <!-- --------------------------------------------- [끝] Book List --------------------------------------------- -->
   <br />
                                          ...
                                          <c:choose>
                                             <c:when test="${sessionScope.mvo.email!=null}">
                                                <h2 class="card__item card__item--small card__title" >
                                                   <a href="showStory.do?book_no=${story.book.book_no}&&storyNo=${story.storyNo}&&email=${sessionScope.mvo.email}" style="font-size: 33px; font-family: 'Nanum Myeongjo'"><b>${story.storyTitle}<b></b></a>
                                                </h2>
                                             </c:when>
                                             <c:otherwise>
                                                <h2 class="card__item card__item--small card__title" style="font-size: 33px; font-family: 'Nanum Myeongjo'">
