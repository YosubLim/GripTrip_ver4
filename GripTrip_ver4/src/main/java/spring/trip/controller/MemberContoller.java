...
public class MemberContoller extends MultiActionController {
   ...
   public ModelAndView updateMember(HttpServletRequest request, HttpServletResponse response, Member pvo)
	...
	//delete 프로필 사진
	@RequestMapping("memberFileDelete.do")
	public ModelAndView memberFileDelete(HttpServletRequest request, HttpServletResponse response, HttpSession session, Story story) throws Exception{
		String memberPictureUrl = request.getParameter("memberPictureUrl");
		String email = request.getParameter("email");
		if(memberPictureUrl!=null){
			memberService.memberFileDelete(memberPictureUrl);
			memberService.deletePictureUrl(email);//null
			Member member = (Member)session.getAttribute("mvo");
			member.setMemberPictureUrl(null);
		}
